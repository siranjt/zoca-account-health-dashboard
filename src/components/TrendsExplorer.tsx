...
type Decliner = { entityId: string; name: string; am: string; latest: number; prev: number; delta: number; tier: string; spark: number[] };
type Trajectory = { d: string; avg: number; green: number; yellow: number; red: number; n: number };
type HealthPayload = { configured: boolean; days: string[]; lookbackWeeks: number; trajectory: Trajectory[]; decliners: Decliner[]; scopedAccounts: number };
        ...
        <>
          {health == null ? (
            ...
            <>
              ...
              <div className="rounded-xl border p-4" style={{ borderColor: "var(--cave-line)", background: "var(--cave-panel)" }}>
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-slate-700">Health decliners</span>
                  <span className="text-xs text-slate-400">— biggest composite drop over the last</span>
                  ...
                  <span className="ml-auto text-xs text-slate-400">{health.decliners.length} sliding</span>
                </div>
                {health.decliners.length === 0 ? (
                  <div className="py-10 text-center text-sm text-slate-400">No accounts declined over the last {lookback} weeks — the book held or improved.</div>
                ) : (
                  <div className="table-scroll -mx-1 max-h-[560px] overflow-auto">
                      ...
                      <tbody>
                        {health.decliners.map((d) => (
                          <tr key={d.entityId} className="border-t border-slate-100">
                            ...
                            <td className="px-2 py-1.5"><Sparkline data={d.spark} color="#dc2626" width={90} height={22} /></td>
                            ...
                            <td className="px-2 py-1.5 text-right tabular-nums font-semibold" style={{ color: "#dc2626" }}>▼ {Math.abs(d.delta).toFixed(1)}</td>
                ...
                <div className="mt-2 text-[11px] text-slate-400">Weekly warehouse health snapshots. A falling line is an account sliding <em>before</em> it churns — the signal a static red list can&apos;t give you.</div>
              </div>
            </>
          )}
        </>
